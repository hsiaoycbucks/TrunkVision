...
它實現了從 **林業調查數據（CSV）** 到 **自動化 3D 建模（Blender Python）**，最後封裝為 **跨平台網頁互動展示（Google <model-viewer>）** 的完整工作流。

使用者無需下載任何 App，即可在 PC 或智慧型手機上流暢閱覽真實比例的立體森林模型。最核心的功能是，透過手機的 AR 模式，可以將這片數據森林直接「種」在現實世界的環境中，實現沉浸式的數據可視化。
...
| 步驟 | 圖示 (建議可放截圖) | 操作說明 |
| :--- | :---: | :--- |
| **1. 掃描與開啟** | (圖片空間) | 使用智慧型手機相機或 LINE 掃描上方的 QR Code，在瀏覽器（建議 Chrome 或 Safari）中開啟網頁。 |
...
| **5. 投影模型** | (圖片空間) | 點擊網格區域，3D 數據森林就會以 1:1 或自動比例投影在現實世界中。 |
...
(例如：本專案採用 MIT 授權條款 - 詳見 LICENSE 檔案)
